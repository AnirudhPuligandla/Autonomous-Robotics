...
while(nnz(value_map) ~= r*c)
    ...
    for i = 1:row(1)
        ...
        %neighbour value
        neighbours = [value_map((X(i)-1),(Y(i)-1)), value_map((X(i)-1),(Y(i))), value_map((X(i)-1),(Y(i)+1)), value_map((X(i)),(Y(i)-1)), value_map((X(i)),(Y(i)+1)), value_map((X(i)+1),(Y(i)-1)), value_map((X(i)+1),(Y(i))), value_map((X(i)+1),(Y(i)+1))];
        
        %Replace all zeros with the next potential value
        neighbours(neighbours==0) = value + 1;
        
        %Update the neighbourhood potential in value_map
        value_map((X(i)-1), (Y(i)-1)) = neighbours(1);
        value_map((X(i)-1), (Y(i))) = neighbours(2);
        value_map((X(i)-1), (Y(i)+1)) = neighbours(3);
        value_map((X(i)), (Y(i)-1)) = neighbours(4);
        value_map((X(i)), (Y(i)+1)) = neighbours(5);
        value_map((X(i)+1), (Y(i)-1)) = neighbours(6);
        value_map((X(i)+1), (Y(i))) = neighbours(7);
        value_map((X(i)+1), (Y(i)+1)) = neighbours(8);
    end
...
if value_map(x, y) ~= 0
    ...
    while 1
        ...
        for i = 1:8
            ...
            % current point
            if x+dx(i) > 0 && x+dx(i) <= r && y+dy(i) > 0 && y+dy(i) <= c && value_map(x+dx(i), y+dy(i)) > 1 && value_map(x+dx(i), y+dy(i)) < value_map(x, y)
                % save the directions
                dir_x = dx(i);
                dir_y = dy(i);
            end;
